...
class BaseTrainer(object):
    def __init__(self, experiment_name, distiller, train_loader, val_loader, cfg, train_set = None):
        self.cfg = cfg
...
class KCDTrainer(BaseTrainer):
    def train(self, resume=False):
        ...
        if resume:
            ...
            self.best_acc = state["best_acc"]
        while epoch < self.cfg.SOLVER.EPOCHS + 1:
            _entropy = -np.ones(self.train_set.init_len())
    ...
    def train_epoch(self, epoch, _entropy):
        ...
        self.distiller.train()
        for idx, data in enumerate(self.train_loader):
            msg = self.train_iter(data, epoch, train_meters, _entropy)
            pbar.set_description(log_msg(msg, "TRAIN"))
            pbar.update()
        pbar.close()
    ...
    def train_iter(self, data, epoch, train_meters, _entropy):
        self.optimizer.zero_grad()
        train_start_time = time.time()
        image, target, index = data
        train_meters["data_time"].update(time.time() - train_start_time)
...
class KCDTrainer_old(BaseTrainer):
    def train(self, resume=False):
        epoch = 1
        if resume:
            state = load_checkpoint(os.path.join(self.log_path, "latest"))
