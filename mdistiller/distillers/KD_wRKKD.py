...
def weighted_kendall(logits_student, logits_teacher):
    ...
    spair = torch.tanh(sr.unsqueeze(-1) - sr.unsqueeze(-2))
    tpair = torch.tanh(tr.unsqueeze(-1) - tr.unsqueeze(-2))

    counts = spair * tpair
    n = sr.size(-1)
    mask = (1 - torch.eye(n, device=sr.device)).unsqueeze(0)
    tau = (counts * pair_weight * mask).sum(dim=(-1, -2)) / (pair_weight * mask).sum(dim=(-1, -2))
...
class wRKKD(Distiller):
    def __init__(self, student, teacher, cfg):
        super(wRKKD, self).__init__(student, teacher)
    ...
    def z_score_norm(self, logits):
        ...
        std = logits.std(dim=1, keepdim=True) + 1e-6 # avoid div by zero
    ...
    def forward_train(self, image_weak, image_strong, target, **kwargs):
        ...
        logits_student_weak = self.z_score_norm(logits_student_weak)
        logits_student_strong = self.z_score_norm(logits_student_strong)
        with torch.no_grad():
        ...
        loss_ = 1 - (self.kd_loss_weight * weighted_kendall(logits_student_weak, logits_teacher_weak)
        + self.kd_loss_weight * weighted_kendall(logits_student_strong, logits_teacher_strong))
