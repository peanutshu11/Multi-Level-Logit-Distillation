...
def kendall(logits_student, logits_teacher):
    ...
    tau = ((counts * mask).sum(dim=(-1, -2))) / (n * (n - 1))
...
class RKKD(Distiller):
    def __init__(self, student, teacher, cfg):
        super(RKKD, self).__init__(student, teacher)
    ...
    def forward_train(self, image_weak, image_strong, target, **kwargs):
        ...
        loss_ = 1 - (self.kd_loss_weight * kendall(logits_student_weak, logits_teacher_weak)
        + self.kd_loss_weight * kendall(logits_student_strong, logits_teacher_strong))
